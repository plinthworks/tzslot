...
| `shift` | `boolean \| ShiftStep \| readonly ShiftOption[]` | `false` | How far one press of an arrow moves the period, and whether there are arrows at all. `false` — the default — draws none: a field that means one chosen period has nothing to step through. `true` draws them and follows what is being chosen: an hour for a period, a day when `singleDay` says it is one date. A step imposes it. A plain number is minutes — `15`, `60`, `1440` — which is what most screens want; anything a number cannot say is said in full: `{ days: 1, minutes: 30 }`, `{ months: 1, hours: 1, minutes: 45 }`, or the short form `'45mn'`. A list offers several and lets the reader pick between them — see `showStep` for whether that picker is on screen. Shortcuts never change it. A shortcut computes a value; a step moves one. |
